...
<?php
  ...
  if ($paymentMethod == "BOL") :
      ...
      $pagamento = $payment->pay($user->get('id'), $transacao->get('id'), $paymentMethod, $transaction->get('total_value'));
      Structure::header();
      echo "<h1>Boleto</h1>";
  ...
  elseif ($paymentMethod == "DEP") :
      ...
      $pagamento = $payment->pay($user->get('id'), $transaction->get('id'), $paymentMethod, $transaction->get('total_value'));
      if ($pagamento) {
          Structure::header();
          echo "<h1>Depósito</h1>";
          $html = "";
          ...
          $html .= '<h3>Valor Total: R$ '.$transaction->get('total_value').'</h3>';
          echo $html;
          echo "<h3>Guarde sempre o seu comprovante de pagamento.</h3>";
          Structure::footer();
      } else {
  ...
  elseif ($paymentMethod == "PPL") :
      ...
      $paypal_html .= '<h3>Valor Total: R$ '.$transaction->get('total_value').'</h3>';
      echo $paypal_html;
